---
name: unity-mcp-playbook
description: Maintain and extend this Unity 6.3 MCP without breaking its scoped write boundary, layering, or TA-focused API strategy.
license: MIT
...
Use this skill when working on this repo's architecture, maintenance, extension planning, or implementation of new Unity MCP capabilities.

This repo is not a general Unity automation tool. It is a focused Unity MCP for material and rendering TA workflows.
...
2. `unity-package/com.ta.unity-mcp/Editor/UnityMcpServer.cs`
   - Unity local HTTP host
   - route wiring
   - main-thread dispatch
   - JSON envelopes

3. `unity-package/com.ta.unity-mcp/Editor/UnityMcpQueries.cs`
   - real Unity query and scoped write logic
   - asset/scene/material/shader inspection
   - the main place to add new retrieval and material-transfer features

4. `unity-package/com.ta.unity-mcp/Editor/UnityShaderGraphTextParser.cs`
   - best-effort Shader Graph structure parsing
   - graph-focused, not compiled-platform analysis

## Non-negotiable constraints

- V1 writes are limited to material migration artifacts under `Assets/`
- No build control
- No playmode control
...
1. Add query logic in `UnityMcpQueries.cs`
2. Expose a Unity HTTP route in `UnityMcpServer.cs`
