...
namespace TA.UnityMcp.Compat.ShaderGraph
{
    internal sealed class GraphSchemaAdapter
    {
        ...
        public List<JObject> ResolveNodeSlots(JObject node, IReadOnlyDictionary<string, JObject> objectMap)
        {
            if (node == null)
            {
            ...
            return ResolveObjectList(node["m_Slots"] ?? node["m_SerializableSlots"], objectMap);
        }

        public JObject FindNodeSlotById(JObject node, int? slotId, IReadOnlyDictionary<string, JObject> objectMap)
        {
            if (node == null || !slotId.HasValue)
            {
                return null;
            }

            return ResolveNodeSlots(node, objectMap)
                .FirstOrDefault(slot => slot.Value<int?>("m_Id") == slotId);
        }

        public string ResolveSlotDirection(JObject slot)
        {
            var slotType = slot?.Value<int?>("m_SlotType");
        ...
        public string ExtractSubGraphGuid(JToken token)
        {
            ...
            if (token.Type == JTokenType.String)
            {
                ...
                try
                {
                    return ExtractSubGraphGuid(JObject.Parse(text));
                }
                catch
                {
                    return text;
            ...
            return token["m_Guid"]?.Value<string>()
                   ?? token["guid"]?.Value<string>()
                   ?? token["subGraph"]?["m_Guid"]?.Value<string>()
                   ?? token["subGraph"]?["guid"]?.Value<string>()
                   ?? token["m_SubGraph"]?["m_Guid"]?.Value<string>()
                   ?? token["m_SubGraph"]?["guid"]?.Value<string>();
        }
